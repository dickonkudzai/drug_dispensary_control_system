<?php
	...
	$sqlP=mysql_query("INSERT INTO prescription(prescription_id,customer_id,customer_name,age,sex,postal_address,invoice_id,phone)VALUES('{$presIdd}','{$c_id}','{$cname}','{$age}','{$sex}','{$postal}','{$invoiceNo}','{$phone}')  ");
	$sqlI=mysql_query("INSERT INTO invoice(invoice_id, customer_name,served_by,status)VALUES('{$invoiceNo}','{$cname}','{$user}','Pending') ");
	...
	while($item1=mysql_fetch_array($getDetails))
		{	
			...
			$sqlId=mysql_query("INSERT INTO invoice_details(invoice,drug,cost,quantity)VALUES('{$invoiceNo}','{$details['stock_id']}','{$details['cost']}','{$item1['quantity']}')");
	...
	while($item12=mysql_fetch_array($getDetails))
		{	
			...
			$sqlIp=mysql_query("INSERT INTO prescription_details(pres_id,drug_name,strength,dose,quantity)VALUES('{$presIdd}','{$details2['stock_id']}','{$item12['strength']}','{$item12['dose']}','{$item12['quantity']}') ");	
...
Drug Analysis Control System,<br> P.O. Box Private Bag Chitungwiza, harare <br> Tel:  <br> E-mail: omega@gmail.com <br>-----------------------------------------
EOD;
...
unset($_SESSION['customer_id'], $_SESSION['customer_name'], $_SESSION['age'], $_SESSION['sex'], $_SESSION['postal_address'], $_SESSION['phone']);
header('Location: add_prescriptions.php');
exit;
