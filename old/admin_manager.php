	...
	if(isset($_POST['submit']))
		{
			...
			$sql1=mysql_query("SELECT * FROM manager WHERE username='$user'")or die(mysql_error());
 			...
 			$dn2 = mysql_num_rows(mysql_query('select manager_id from manager'));
			$sid = $dn2+1;
		 	$staff_id = "manager/".$sid;
 			if($result>0)
 				{
					$message="<font color=blue>sorry the username entered already exists</font>";
 					...
 					{
						$sql=mysql_query("INSERT INTO manager(first_name,last_name,staff_id,postal_address,phone,email,username,password,date)VALUES('$fname','$lname','$staff_id','$postal','$phone','$email','$user','$pas',NOW())");
						if($sql>0)
							{
